...
public class LinkedStack<E> implements Cloneable
{
   ...
   **/ 
   ...
   /**
   * Accessor method that returns data from a certain location in the stack
   * @ param - n
   *  The location to where the information will be retrived
   * @ return
   *  The data at the specific location
   * @ exception - NoSuchElementException
   *  Indicates that n is either negative or n is greater than the stack size
   **/
   public Object itemAt(int n){
      
      // Throw an exception of n is out of range
      if (n < 0 || n > size()){
         throw new NoSuchElementException("n is either negative or n is greater than the stack size. The itemAt method cannot be used!");
      }
      return (Node.listPosition(top, n).getData());
   }          
}